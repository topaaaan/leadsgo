...
class ReportMarketingRslScreen extends StatefulWidget {
  @override
  _ReportMarketingRslScreen createState() => _ReportMarketingRslScreen();

  String branch;
  String namaCabang;

  ReportMarketingRslScreen(this.branch, this.namaCabang);
}
...
class _ReportMarketingRslScreen extends State<ReportMarketingRslScreen> {
  void launchWhatsApp({
  ...
  final String apiUrl =
      'https://tetranabasainovasi.com/api_marsit_v1/service.php/getMarketingRslReport';
  List<dynamic> _users = [];
  ...
  void fetchUsers() async {
    ...
    var result = await http.post(Uri.parse(apiUrl), body: {'branch': widget.branch});
    if (result.statusCode == 200) {
      setState(() {
        if (json.decode(result.body)['Daftar_Report_Marketing_Rsl'] == '') {
          _isLoading = false;
        } else {
          _users = json.decode(result.body)['Daftar_Report_Marketing_Rsl'];
          _isLoading = false;
        }
      });
    }
                          ...
                          children: <Widget>[
                            ...
                            Row(
                              children: <Widget>[
                                ...
                                Text(
                                  jabatan(_jabatan(_users[index]), _statusKaryawan(_users[index])),
                                  style: TextStyle(
                                    fontFamily: 'LeadsGo-Font',
                                    fontWeight: FontWeight.bold,
                                  ),
  ...
  jabatan(String jabatan, String status) {
    if (jabatan == '81') {
      if (status == '5') {
        return 'MA';
      } else {
